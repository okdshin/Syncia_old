#ifdef SPREADKEYHASHBEHAVIOR_UNIT_TEST
...
int main(int argc, char* argv[])
{
	...
	auto spread_key_hash_behavior = 
		SpreadKeyHashBehavior::Create(node_id, max_key_hash_count, max_hop_count, 
			upper_pool, file_db, std::cout);	
	spread_key_hash_behavior->Bind(dispatcher);
	...
	auto spread_key_hash_action = 
		SpreadKeyHashAction::Create(upper_pool, node_id, std::cout);
	shell.Register("fetch", ": fetch",
		[spread_key_hash_action]
		(const nr::utl::Shell::ArgumentList& argument_list){
			spread_key_hash_action->RequestSpreadKeyHash();
		});
